...
public class Heuristique1 extends Heuristique{
	...
	public void solve() {
		/*On choisit l'efficacité minimale que doit avoir un routeur
		 * pour qu'il soit eligible à être placé
		*/
		int eff = 5;
		int cost = 0;
			...
			for (int i=0; i<rf.getNbRow(); i++) {
				if(!instance[i][j].getStatut().equals("#")){
					...
					if( listCoord.size()>eff) {
						...
						//On recupère la cellule connectée au backbone la plus proche de la colonne
						...
						if(cost + prixInstall <= rf.getBudget() && prixInstall < 1000 * maxlength){
							...
							//couvre les cellules couvertes par le nouveau routeur placé
